...
def create_windows(data, window_size=128, stride=32):
    """
...
def compute_gaussian_params(window):
    """
    Estimate multivariate Gaussian parameters.
...
def wasserstein_gaussian(mu1, cov1, mu2, cov2):
    """
    ...
    cov_term = np.trace(
        cov1 + cov2 - 2 * middle_sqrt
    )

    w2 = mean_term + cov_term

    return np.sqrt(max(w2, 0))



def compute_nonstationarity(train,
                            window_size=128,
                            stride=32):

    """
    Compute normal distribution shift.
    ...
    # 2. estimate Gaussian for each window
    gaussian_list = []

    for w in windows:
        mu, cov = compute_gaussian_params(w)
        gaussian_list.append((mu, cov))


    # 3. compute W2 between consecutive windows
    distances = []

    for i in range(len(gaussian_list)-1):

        mu1, cov1 = gaussian_list[i]
        mu2, cov2 = gaussian_list[i+1]

        d = wasserstein_gaussian(
            mu1, cov1,
            mu2, cov2
        )
    ...
    return {
        "mean_W2": np.mean(distances),
        "std_W2": np.std(distances),
